...
pub fn simulate_guaranteed_thirds(matches: &[Match]) -> ThirdPlaceSimulation {
    ...
    let mut qualified_counts: HashMap<String, u64> = HashMap::new();
    let mut all_third_teams: HashSet<String> = HashSet::new();
    ...
    for group_code in GROUP_CODES {
        let teams: HashSet<String> = matches
            ...
            .collect();
        for t in teams {
            all_third_teams.insert(t.clone());
            qualified_counts.entry(t).or_insert(0);
        }
    }
    ...
    if total_exhaustive > 0 && total_exhaustive <= MAX_EXHAUSTIVE_SCENARIOS {
        ...
        simulate_exhaustive(&matches, &mut current, &unplayed, 0, &mut qualified_counts);
    } else {
        method = format!("monte-carlo ({MONTE_CARLO_SAMPLES} samples)");
        total_scenarios = MONTE_CARLO_SAMPLES;
        simulate_monte_carlo(matches, &unplayed, MONTE_CARLO_SAMPLES, &mut qualified_counts);
    }

    let mut team_info: HashMap<String, (Team, GroupCode)> = HashMap::new();
    for m in matches {
        for team in [&m.home_team, &m.away_team] {
            team_info.entry(team.fifa_code.clone()).or_insert_with(|| (team.clone(), m.group.clone()));
        }
    }
    ...
    for fifa_code in &all_third_teams {
        let count = qualified_counts.get(fifa_code).copied().unwrap_or(0);
        let (team, group) = team_info[fifa_code].clone();

        let group_matches: Vec<&Match> = matches.iter().filter(|m| m.group.0 == group.0).collect();
        let standings = calculate_standings(&group_matches.iter().cloned().cloned().collect::<Vec<_>>());
        ...
        if count == total_scenarios {
            ...
            uncertain.push(ThirdPlaceChance {
                team,
                group,
                scenarios_qualified: count,
                percentage: if total_scenarios > 0 {
                    (count as f64 / total_scenarios as f64) * 100.0
                } else {
                    0.0
                },
                points,
...
fn simulate_exhaustive(
    ...
    unplayed: &[usize],
    idx: usize,
    qualified_counts: &mut HashMap<String, u64>,
) {
    if idx == unplayed.len() {
        let third_places = rank_third_places(&group_standings(current));
        for (_, standing) in third_places.iter().take(TOP_THIRDS) {
            *qualified_counts.get_mut(&standing.team.fifa_code).unwrap() += 1;
        }
        return;
    }
    ...
    for result in &results {
        ...
        simulate_exhaustive(original, current, unplayed, idx + 1, qualified_counts);
    }
...
fn simulate_monte_carlo(
    ...
    for s in 0..samples {
        ...
        let third_places = rank_third_places(&group_standings(&current));
        for (_, standing) in third_places.iter().take(TOP_THIRDS) {
            if let Some(count) = qualified_counts.get_mut(&standing.team.fifa_code) {
                *count += 1;
            }
        }
    }
